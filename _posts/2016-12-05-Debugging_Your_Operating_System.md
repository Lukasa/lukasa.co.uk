...
What is potentially more surprising is that the same optimisation can be made for `calloc`. This is because the operating system keeps hold of many pre-zeroed pages of memory. This is usually done in the background: when the computer has nothing else to do and has pages of memory that have been freed by previously-running processes, the kernel will spend some CPU cycles zeroing that memory. This is very important: one of the important illusions that virtual memory needs to provide is the illusion that no other process is running in your memory space, and that means that *data* from other processes must not leak into yours! It would be a pretty nasty security bug if it were possible for a memory page freed by process A to be used by process B without the kernel scrubbing all the data out first!

But because the OS has a list of pre-zeroed memory pages already floating around, that means that `calloc` can, for larger allocations, simply do the same as `malloc` does, and ask for brand-new virtual memory pages. This continues to have no cost until the memory is actually used. This neat optimisation means that `calloc(1024 * 1024 * 1024, 1)` costs exactly the same as `malloc` of the same size does, despite `calloc`'s additional promise of zeroing memory. Neat!
